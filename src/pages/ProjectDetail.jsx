  ...
  return (
    <div className="relative w-full bg-white min-h-screen overflow-hidden font-sans">
      ...
      <div className="relative z-20 max-w-6xl mx-auto px-6 md:px-12 pt-28 pb-32">
        ...
        {/* Hero cover — same browser-chrome frame, object-contain, full
            screenshot always visible, no cropping */}
        <motion.div
          initial={{ opacity: 0, y: 40, scale: 0.98 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          transition={{ delay: 0.34, duration: 0.8, ease: easeOut }}
          className="rounded-3xl p-2 mb-20
        ...
        {/* Gallery — same browser-chrome frame per shot, full screenshots
            always visible instead of cropped 4:3 tiles */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-24">
          {project.gallery.map((src, i) => (
            <motion.div
              ...
              whileHover={{ y: -6 }}
              className="rounded-2xl p-1.5 bg-white border border-gray-200 shadow-[0_15px_40px_-15px_rgba(0,0,0,0.15)]"
            >
              <ScreenshotFrame src={src} alt={`${project.title} screenshot ${i + 1}`} />
            </motion.div>
          ))}
        </div>
